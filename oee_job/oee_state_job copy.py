...
#     """
#     Upsert từng event. Yêu cầu fact_state_event có UNIQUE KEY để tránh trùng:
#       UNIQUE KEY uq_line_start (line_id, start_ts)
#     """
#     if ev.empty:
...
#             # --- GÁN reason_id nếu là downtime (tùy dữ liệu, có thể bỏ điều kiện) ---
#             reason_id = None
#             try:
#                 # Nếu bạn muốn bỏ qua RUN/IDLE thì thêm điều kiện ở đây,
#                 # if int(r["state_id"]) not in (STATE_MAP["RUN"], STATE_MAP["IDLE"]):
#                 q = conn.execute(
#                     REASON_LOOKUP_SQL,
#                     {
...
#                 logging.warning("Lookup reason_id failed (line=%s, %s..%s): %s",
#                                 r["line_id"], r["start_ts"], r["end_ts"], ex)

#             # Nếu trong DataFrame đã có cột reason_id (None), ưu tiên giá trị vừa tìm
#             payload_reason_id = reason_id
#             if "reason_id" in r and pd.notna(r["reason_id"]):
#                 payload_reason_id = int(r["reason_id"])

#             # --- UPSERT ---
#             conn.execute(UPSERT_SQL, {
#                 "line_id":  int(r["line_id"]),
#                 "state_id": int(r["state_id"]),
#                 "reason_id": payload_reason_id,  # đã gán ở trên (có thể là None)
#                 "start_ts": r["start_ts"],
#                 "end_ts":   r["end_ts"],
#                 "note": None if pd.isna(r.get("note")) else str(r.get("note")),
#             })
