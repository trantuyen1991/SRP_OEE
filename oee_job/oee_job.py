...
def load_raw_since(lineId:int,since_dt: datetime) -> pd.DataFrame:
    """
    ...
    sql = text("""
        ...
        ORDER BY ts
    """)
    df = pd.read_sql(sql, ENGINE, params={"line_id": lineId, "since": since_dt})
    if not df.empty:
        df['ts'] = pd.to_datetime(df['ts'])
    logging.info("Line-%d Loaded %d rows raw data since %s", lineId, len(df), since_dt)
    return df


def compute_minute(lineId:int,df: pd.DataFrame) -> pd.DataFrame:
    """
...
def main() -> None:
    ...
    try:
        ...
        line_ids = [103, 104, 105, 106, 107]
        for line_id in line_ids:
            raw = load_raw_since(lineId=line_id,since_dt=since)
            dfm = compute_minute(lineId=line_id,df=raw)
            upsert_minute(dfm)
