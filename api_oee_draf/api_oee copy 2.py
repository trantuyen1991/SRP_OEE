...
- /oee/line/{line_id}: hỗ trợ range mode (from_ts/to_ts) và gran mode (min, shift, day...).
...
app=FastAPI(title="OEE API",version="0.3.0")
...
def get_oee(line_id:int,q:FilterParams=Depends(_normalize_params))->JSONResponse:
    gran=q.gran or "min"; limit=q.limit or TARGET_POINTS; bucket_sec=q.bucket_sec
    if q.from_ts and q.to_ts:
        dt_from=_parse_iso(q.from_ts); dt_to=_parse_iso(q.to_ts)
        if dt_from>=dt_to: raise HTTPException(status_code=400,detail="from_ts must < to_ts")
        key=("range",line_id,q.process_order,q.packaging_id,q.from_ts,q.to_ts,limit,bucket_sec)
        cached=CACHE.get(key); 
        if cached: return JSONResponse(cached)
        sql_s,p_s,sql_g,p_g,bucket_used=_build_sql_range(line_id,q.process_order,q.packaging_id,dt_from,dt_to,limit,bucket_sec)
        with ENGINE.connect() as conn:
            df_series=pd.read_sql(text(sql_s),conn,params=p_s); df_gauges=pd.read_sql(text(sql_g),conn,params=p_g)
        data=_df_to_records(df_series); gauges=_df_to_records(df_gauges); gauges=gauges[0] if gauges else {}
        payload={"meta":{"mode":"range","line_id":line_id,"from_ts":q.from_ts,"to_ts":q.to_ts,
            "process_order":q.process_order,"packaging_id":q.packaging_id,"row_count":len(data),
            "limit":limit,"bucket_sec":bucket_used,"generated_at":datetime.now().strftime("%Y-%m-%d %H:%M:%S")},
            "gauges":gauges,"data":data}
        CACHE.set(key,payload); return JSONResponse(payload)
    dt_to=datetime.now(); since=q.since_min or 240; dt_from=dt_to-timedelta(minutes=since)
    ...
    with ENGINE.connect() as conn:
        df_series=pd.read_sql(text(sql_s),conn,params=p_s); df_gauges=pd.read_sql(text(sql_g),conn,params=p_g)
    data=_df_to_records(df_series); gauges=_df_to_records(df_gauges); gauges=gauges[0] if gauges else {}
    payload={"meta":{"mode":"gran","gran":gran,"line_id":line_id,"since_min":since,"row_count":len(data),
        "limit":limit,"bucket_sec":bucket_used,"process_order":q.process_order,"packaging_id":q.packaging_id,
...
def get_dashboard(line_id:int,q:FilterParams=Depends(_normalize_params))->JSONResponse:
    limit=q.limit or TARGET_POINTS; bucket_sec=q.bucket_sec; now=datetime.now()
    if q.from_ts and q.to_ts: dt_from=_parse_iso(q.from_ts); dt_to=_parse_iso(q.to_ts)
    else: since=q.since_min or 240; dt_to=now; dt_from=now-timedelta(minutes=since)
    if dt_from>=dt_to: raise HTTPException(status_code=400,detail="from_ts must < to_ts")
    key=("dashboard",line_id,q.process_order,q.packaging_id,dt_from,dt_to,limit,bucket_sec)
    cached=CACHE.get(key); 
    if cached: return JSONResponse(cached)
    sql_s,p_s,sql_g,p_g,bucket_used=_build_sql_range(line_id,q.process_order,q.packaging_id,dt_from,dt_to,limit,bucket_sec)
    p_common={"line_id":line_id,"from_ts":dt_from.strftime("%Y-%m-%d %H:%M:%S"),"to_ts":dt_to.strftime("%Y-%m-%d %H:%M:%S")}
    # shift
    sql_shift="""SELECT s.shift_date,s.shift_no,
      ...
      ROUND(100.0*SUM(f.good)/NULLIF(SUM(f.produced),0),2) AS quality_pct,
      ROUND((SUM(f.runtime_sec)/NULLIF(SUM(f.planned_sec),0))*(SUM(f.produced)/NULLIF((SUM(f.runtime_sec)/60.0)*COALESCE(dp.ideal_rate_per_min,0)),0))*(SUM(f.good)/NULLIF(SUM(f.produced),0))*100,2) AS oee_pct
    FROM dim_shift_calendar s JOIN fact_production_min f
      ON f.ts_min>=CONCAT(s.shift_date,' ',s.start_time)
     AND f.ts_min<DATE_ADD(CONCAT(s.shift_date,' ',s.end_time),INTERVAL (s.end_time<s.start_time) DAY)
    LEFT JOIN dim_packaging dp ON dp.packaging_id=f.packaging_id
    WHERE f.line_id=:line_id AND f.ts_min BETWEEN :from_ts AND :to_ts
    GROUP BY s.shift_date,s.shift_no ORDER BY s.shift_date,s.shift_no"""
    # day
    sql_day="""SELECT DATE(f.ts_min) AS bucket,
      SUM(f.produced) AS produced,SUM(f.good) AS good,SUM(f.ng) AS ng,
      ROUND(100.0*SUM(f.runtime_sec)/NULLIF(SUM(f.planned_sec),0),2) AS availability_pct,
      ROUND(100.0*SUM(f.produced)/NULLIF(SUM((f.runtime_sec)/60.0)*COALESCE(dp.ideal_rate_per_min,0)),2) AS performance_pct,
      ROUND(100.0*SUM(f.good)/NULLIF(SUM(f.produced),0),2) AS quality_pct,
      ROUND((SUM(f.runtime_sec)/NULLIF(SUM(f.planned_sec),0))*(SUM(f.produced)/NULLIF((SUM(f.runtime_sec)/60.0)*COALESCE(dp.ideal_rate_per_min,0)),0))*(SUM(f.good)/NULLIF(SUM(f.produced),0))*100,2) AS oee_pct
    FROM fact_production_min f LEFT JOIN dim_packaging dp ON dp.packaging_id=f.packaging_id
    WHERE f.line_id=:line_id AND f.ts_min BETWEEN :from_ts AND :to_ts
    GROUP BY DATE(f.ts_min) ORDER BY DATE(f.ts_min)"""
    # week
    sql_week="""SELECT YEARWEEK(f.ts_min,3) AS bucket, SUM(f.produced) AS produced,SUM(f.good) AS good,SUM(f.ng) AS ng,
      ROUND(100.0*SUM(f.runtime_sec)/NULLIF(SUM(f.planned_sec),0),2) AS availability_pct,
      ROUND(100.0*SUM(f.produced)/NULLIF(SUM((f.runtime_sec)/60.0)*COALESCE(dp.ideal_rate_per_min,0)),2) AS performance_pct,
      ROUND(100.0*SUM(f.good)/NULLIF(SUM(f.produced),0),2) AS quality_pct,
      ROUND((SUM(f.runtime_sec)/NULLIF(SUM(f.planned_sec),0))*(SUM(f.produced)/NULLIF((SUM(f.runtime_sec)/60.0)*COALESCE(dp.ideal_rate_per_min,0)),0))*(SUM(f.good)/NULLIF(SUM(f.produced),0))*100,2) AS oee_pct
    FROM fact_production_min f LEFT JOIN dim_packaging dp ON dp.packaging_id=f.packaging_id
    WHERE f.line_id=:line_id AND f.ts_min BETWEEN :from_ts AND :to_ts
    GROUP BY YEARWEEK(f.ts_min,3) ORDER BY YEARWEEK(f.ts_min,3)"""
    # month
    sql_month="""SELECT DATE_FORMAT(f.ts_min,'%Y-%m') AS bucket, SUM(f.produced) AS produced,SUM(f.good) AS good,SUM(f.ng) AS ng,
      ROUND(100.0*SUM(f.runtime_sec)/NULLIF(SUM(f.planned_sec),0),2) AS availability_pct,
      ROUND(100.0*SUM(f.produced)/NULLIF(SUM((f.runtime_sec)/60.0)*COALESCE(dp.ideal_rate_per_min,0)),2) AS performance_pct,
      ROUND(100.0*SUM(f.good)/NULLIF(SUM(f.produced),0),2) AS quality_pct,
      ROUND((SUM(f.runtime_sec)/NULLIF(SUM(f.planned_sec),0))*(SUM(f.produced)/NULLIF((SUM(f.runtime_sec)/60.0)*COALESCE(dp.ideal_rate_per_min,0)),0))*(SUM(f.good)/NULLIF(SUM(f.produced),0))*100,2) AS oee_pct
    FROM fact_production_min f LEFT JOIN dim_packaging dp ON dp.packaging_id=f.packaging_id
    WHERE f.line_id=:line_id AND f.ts_min BETWEEN :from_ts AND :to_ts
    GROUP BY DATE_FORMAT(f.ts_min,'%Y-%m') ORDER BY bucket"""
    # quarter
    sql_quarter="""SELECT CONCAT(YEAR(f.ts_min),'-Q',QUARTER(f.ts_min)) AS bucket,
      SUM(f.produced) AS produced,SUM(f.good) AS good,SUM(f.ng) AS ng,
      ROUND(100.0*SUM(f.runtime_sec)/NULLIF(SUM(f.planned_sec),0),2) AS availability_pct,
      ROUND(100.0*SUM(f.produced)/NULLIF(SUM((f.runtime_sec)/60.0)*COALESCE(dp.ideal_rate_per_min,0)),2) AS performance_pct,
      ROUND(100.0*SUM(f.good)/NULLIF(SUM(f.produced),0),2) AS quality_pct,
    ...
    FROM fact_production_min f LEFT JOIN dim_packaging dp ON dp.packaging_id=f.packaging_id
    WHERE f.line_id=:line_id AND f.ts_min BETWEEN :from_ts AND :to_ts
    GROUP BY YEAR(f.ts_min),QUARTER(f.ts_min) ORDER BY YEAR(f.ts_min),QUARTER(f.ts_min)"""
    # year
    sql_year="""SELECT YEAR(f.ts_min) AS bucket, SUM(f.produced) AS produced,SUM(f.good) AS good,SUM(f.ng) AS ng,
      ROUND(100.0*SUM(f.runtime_sec)/NULLIF(SUM(f.planned_sec),0),2) AS availability_pct,
      ROUND(100.0*SUM(f.produced)/NULLIF(SUM((f.runtime_sec)/60.0)*COALESCE(dp.ideal_rate_per_min,0)),2) AS performance_pct,
      ROUND(100.0*SUM(f.good)/NULLIF(SUM(f.produced),0),2) AS quality_pct,
      ROUND((SUM(f.runtime_sec)/NULLIF(SUM(f.planned_sec),0))*(SUM(f.produced)/NULLIF((SUM(f.runtime_sec)/60.0)*COALESCE(dp.ideal_rate_per_min,0)),0))*(SUM(f.good)/NULLIF(SUM(f.produced),0))*100,2) AS oee_pct
    FROM fact_production_min f LEFT JOIN dim_packaging dp ON dp.packaging_id=f.packaging_id
    WHERE f.line_id=:line_id AND f.ts_min BETWEEN :from_ts AND :to_ts
    GROUP BY YEAR(f.ts_min) ORDER BY YEAR(f.ts_min)"""
    with ENGINE.connect() as conn:
        df_series=pd.read_sql(text(sql_s),conn,params=p_s); df_gauges=pd.read_sql(text(sql_g),conn,params=p_g)
        df_shift=pd.read_sql(text(sql_shift),conn,params=p_common); df_day=pd.read_sql(text(sql_day),conn,params=p_common)
        df_week=pd.read_sql(text(sql_week),conn,params=p_common); df_month=pd.read_sql(text(sql_month),conn,params=p_common)
        df_quarter=pd.read_sql(text(sql_quarter),conn,params=p_common); df_year=pd.read_sql(text(sql_year),conn,params=p_common)
    data=_df_to_records(df_series); gauges=_df_to_records(df_gauges); gauges=gauges[0] if gauges else {}
    payload={"meta":{"mode":"dashboard","line_id":line_id,"from_ts":dt_from.strftime("%Y-%m-%d %H:%M:%S"),
        "to_ts":dt_to.strftime("%Y-%m-%d %H:%M:%S"),"process_order":q.process_order,"packaging_id":q.packaging_id,
        "row_count":len(data),"limit":limit,"bucket_sec":bucket_used,"generated_at":now.strftime("%Y-%m-%d %H:%M:%S")},
        "gauges":gauges,"linechart":data,"shift_summary":_df_to_records(df_shift),"day_summary":_df_to_records(df_day),
        "week_summary":_df_to_records(df_week),"month_summary":_df_to_records(df_month),
        "quarter_summary":_df_to_records(df_quarter),"year_summary":_df_to_records(df_year)}
    CACHE.set(key,payload); return JSONResponse(payload)
